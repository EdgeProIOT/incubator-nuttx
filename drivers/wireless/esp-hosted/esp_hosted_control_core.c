/****************************************************************************
 * drivers/wireless/esp-hosted/esp_hosted_control_core.c
 *
...
#include "esp_hosted_serial_if.h"
...
struct esp_hosted_queue_s* ctrl_msg_Q = NULL;
  ...
  else if (proto_msg->msg_type == CTRL_MSG_TYPE__Resp)
    {
      ...
      if (CALLBACK_AVAILABLE ==
      ...
      else
        {
          ...
          elem = (struct q_element_s*)malloc(sizeof(struct q_element_s));
          if (!elem)
            {
          ...
          if (esp_hosted_queue_put(ctrl_msg_Q, (void*)elem))
            {
...
static ctrl_cmd_t * get_response(int *read_len, int timeout_sec)
{
  ...
  data = esp_hosted_queue_get(ctrl_msg_Q);
  if (data)
    {
      elem = (struct q_element_s*)data;
      if (!elem)
        return NULL;
...
int deinit_hosted_control_lib_internal(void)
{
  ...
  if (ctrl_msg_Q)
    {
      esp_hosted_queue_destroy(&ctrl_msg_Q);
    }
...
int init_hosted_control_lib_internal(void)
{
  ...
  /* queue init */
  ctrl_msg_Q = esp_hosted_queue_create();
  if (!ctrl_msg_Q)
    {
