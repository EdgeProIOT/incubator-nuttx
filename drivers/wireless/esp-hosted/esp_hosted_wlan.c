/****************************************************************************
 * drivers/wireless/esp-hosted/esp_hosted_wlan.c
 *
...
static struct wlan_priv_s g_wlan_priv[ESP_HOSTED_WLAN_DEVS];

#ifdef ESP_HOSTED_WLAN_HAS_STA
static const struct wlan_ops g_sta_ops =
{
  .start      = esp_hosted_wifi_sta_start,
  .send       = esp_hosted_wifi_sta_send_data,
  .essid      = esp_hosted_wifi_sta_essid,
  .bssid      = esp_hosted_wifi_sta_bssid,
  .passwd     = esp_hosted_wifi_sta_password,
  .mode       = esp_hosted_wifi_sta_mode,
  .auth       = esp_hosted_wifi_sta_auth,
  .freq       = esp_hosted_wifi_sta_freq,
  .bitrate    = esp_hosted_wifi_sta_bitrate,
  .txpower    = esp_hosted_wifi_sta_txpower,
  .channel    = esp_hosted_wifi_sta_channel,
  .country    = esp_hosted_wifi_sta_country,
  .rssi       = esp_hosted_wifi_sta_rssi,
  .connect    = esp_hosted_wifi_sta_connect,
  .disconnect = esp_hosted_wifi_sta_disconnect,
  .event      = esp_hosted_wifi_notify_subscribe,
  .stop       = esp_hosted_wifi_sta_stop
};
...
{
  .start      = esp_hosted_wifi_softap_start,
  .send       = esp_hosted_wifi_softap_send_data,
  .essid      = esp_hosted_wifi_softap_essid,
  .bssid      = esp_hosted_wifi_softap_bssid,
  .passwd     = esp_hosted_wifi_softap_password,
  .mode       = esp_hosted_wifi_softap_mode,
  .auth       = esp_hosted_wifi_softap_auth,
  .freq       = esp_hosted_wifi_softap_freq,
  .bitrate    = esp_hosted_wifi_softap_bitrate,
  .txpower    = esp_hosted_wifi_softap_txpower,
  .channel    = esp_hosted_wifi_softap_channel,
  .country    = esp_hosted_wifi_softap_country,
  .rssi       = esp_hosted_wifi_softap_rssi,
  .connect    = esp_hosted_wifi_softap_connect,
  .disconnect = esp_hosted_wifi_softap_disconnect,
  .event      = esp_hosted_wifi_notify_subscribe,
  .stop       = esp_hosted_wifi_softap_stop
};
...
static int wlan_ifup(struct net_driver_s *dev)
{
  ...
  if (g_callback_register_ref == 0)
    {
      ret = esp_hosted_register_shutdown_handler(esp_wifi_stop_callback);
      if (ret < 0)
        {
...
static int wlan_ifdown(struct net_driver_s *dev)
{
  ...
  if (g_callback_register_ref == 0)
    {
      ret = esp_hosted_unregister_shutdown_handler(esp_wifi_stop_callback);
      if (ret < 0)
        {
                      ...
                      unsigned long arg)
{
  ...
  switch (cmd)
    ...
    case SIOCSIWSCAN:
      ret = esp_hosted_wifi_start_scan(iwr);
      break;

    case SIOCGIWSCAN:
      ret = esp_hosted_wifi_get_scan_results(iwr);
      break;
...
/****************************************************************************
 * Name: esp_hosted_net_initialize
 *
 * Description:
 *   Initialize the ESP-Hosted driver
 *
 * Input Parameters:
...
static int esp_hosted_net_initialize(int devno, uint8_t *mac_addr,
...
{
  struct wlan_priv_s *priv = &g_wlan_priv[ESP_HOSTED_WLAN_STA_DEVNO];
...
{
  struct wlan_priv_s *priv = &g_wlan_priv[ESP_HOSTED_WLAN_SOFTAP_DEVNO];
...
int esp_hosted_wlan_sta_initialize(void)
{
  ...
  ret = esp_hosted_wifi_adapter_init();
  if (ret < 0)
    {
  ...
  ret = esp_hosted_wifi_sta_read_mac(eth_mac);
  if (ret < 0)
    {
  ...
  ret = esp_hosted_net_initialize(ESP_HOSTED_WLAN_STA_DEVNO, eth_mac, &g_sta_ops);
  if (ret < 0)
    {
  ...
  ret = esp_hosted_wifi_sta_register_recv_cb(wlan_sta_rx_done);
  if (ret < 0)
    {
  ...
  esp_hosted_wifi_sta_register_txdone_cb(wlan_sta_tx_done);
...
int esp_hosted_wlan_softap_initialize(void)
{
  ...
  ret = esp_hosted_wifi_softap_read_mac(eth_mac);
  if (ret < 0)
    {
      nerr("ERROR: Failed to read MAC address\n");
      return ret;
  ...
  ret = esp_hosted_net_initialize(ESP_HOSTED_WLAN_SOFTAP_DEVNO, eth_mac,
                                  &g_softap_ops);
  if (ret < 0)
    {
      nerr("ERROR: Failed to initialize net\n");
  ...
  ret = esp_hosted_wifi_softap_register_recv_cb(wlan_softap_rx_done);
  if (ret < 0)
    {
      nerr("ERROR: Failed to register RX callback\n");
      return ret;
  ...
  esp_hosted_wifi_softap_register_txdone_cb(wlan_softap_tx_done);
