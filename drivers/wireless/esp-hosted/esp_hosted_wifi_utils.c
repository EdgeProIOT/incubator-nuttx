/****************************************************************************
 * drivers/wireless/esp-hosted/esp_hosted_wifi_utils.c
 *
...
#ifdef CONFIG_ESP_HOSTED_WIFI_SCAN_RESULT_SIZE
#  define WIFI_SCAN_RESULT_SIZE      CONFIG_ESP_HOSTED_WIFI_SCAN_RESULT_SIZE
#else
...
/****************************************************************************
 * Name: esp_hosted_wifi_start_scan
 *
 * Description:
...
int esp_hosted_wifi_start_scan(struct iwreq *iwr)
{
  ...
  esp_hosted_wifi_start();
  ret = esp_hosted_wifi_scan_start(config, false);
  if (ret != OK)
    {
...
/****************************************************************************
 * Name: esp_hosted_wifi_get_scan_results
 *
 * Description:
 *   Get scan result
 *
...
int esp_hosted_wifi_get_scan_results(struct iwreq *iwr)
{
  int ret = OK;
...
/****************************************************************************
 * Name: esp_hosted_wifi_scan_event_parse
 *
 * Description:
...
void esp_hosted_wifi_scan_event_parse(void)
{
  struct wifi_scan_result *priv = &g_scan_priv;
  ...
  esp_hosted_wifi_scan_get_ap_num(&bss_total);
  if (bss_total == 0)
    {
  ...
  if (esp_hosted_wifi_scan_get_ap_records(&bss_total,
