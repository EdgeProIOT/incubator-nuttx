/****************************************************************************
 * drivers/wireless/esp-hosted/esp_hosted_wifi_adapter.c
 *
...
#include "esp_hosted_wifi_adapter.h"
#include "esp_hosted_wifi_utils.h"
#include "esp_hosted_wlan.h"
...
static void esp_hosted_set_isr(int32_t n, void *f, void *arg);
static void esp_hosted_ints_on(uint32_t mask);
static void esp_hosted_ints_off(uint32_t mask);
static uint32_t esp_hosted_wifi_int_disable(void *wifi_int_mux);
static void esp_hosted_wifi_int_restore(void *wifi_int_mux, uint32_t tmp);
...
static int32_t esp_hosted_wifi_read_mac(uint8_t *mac, uint32_t type);
...
static void *esp_hosted_wifi_malloc(size_t size);
static void *esp_hosted_wifi_realloc(void *ptr, size_t size);
static void *esp_hosted_wifi_calloc(size_t n, size_t size);
static void *esp_hosted_wifi_zalloc(size_t size);
static void *esp_hosted_wifi_create_queue(int32_t queue_len, int32_t item_size);
static void esp_hosted_wifi_delete_queue(void *queue);
...
static int esp_hosted_int_adpt_cb(int irq, void *context, void *arg)
{
...
static int esp_hosted_wifi_lock(bool lock)
{
  int ret;
...
static void esp_hosted_set_isr(int32_t n, void *f, void *arg)
{
  ...
  ret = irq_attach(irq, esp_hosted_int_adpt_cb, adapter);
  if (ret)
    {
...
/****************************************************************************
 * Name: esp_hosted_wifi_int_disable
 *
 * Description:
...
static uint32_t IRAM_ATTR esp_hosted_wifi_int_disable(void *wifi_int_mux)
{
  irqstate_t flags;
...
/****************************************************************************
 * Name: esp_hosted_wifi_int_restore
 *
 * Description:
...
static void IRAM_ATTR esp_hosted_wifi_int_restore(void *wifi_int_mux, uint32_t tmp)
{
  ...
  while (1)
    {
      ...
      esp_hosted_wifi_lock(true);

      switch (evt_adpt->id)
        {
        case WIFI_ADPT_EVT_SCAN_DONE:
          esp_hosted_wifi_scan_event_parse();
          break;
        ...
        case WIFI_ADPT_EVT_STA_START:
          ...
          ret = esp_hosted_wifi_set_ps(WIFI_PS_NONE);
          if (ret)
            {
          ...
          ret = esp_hosted_wifi_get_config(WIFI_IF_STA, &g_sta_wifi_cfg);
          if (ret)
            {
        ...
        case WIFI_ADPT_EVT_AP_START:
          ...
          ret = esp_hosted_wifi_get_config(WIFI_IF_AP, &g_softap_wifi_cfg);
          if (ret)
            {
              wlerr("Failed to get Wi-Fi config data ret=%d\n", ret);
            }
...
/****************************************************************************
 * Name: esp_hosted_wifi_create_queue
 *
 * Description:
...
/****************************************************************************
 * Name: esp_hosted_wifi_delete_queue
 *
 * Description:
...
/****************************************************************************
 * Name: esp_hosted_wifi_tx_done_cb
 *
 * Description:
...
static IRAM_ATTR void esp_hosted_wifi_tx_done_cb(uint8_t ifidx, uint8_t *data,
...
/****************************************************************************
 * Name: esp_hosted_wifi_auth_trans
 *
 * Description:
...
static int esp_hosted_wifi_auth_trans(uint32_t wifi_auth)
{
...
/****************************************************************************
 * Name: esp_hosted_wifi_cipher_trans
 *
 * Description:
...
static int esp_hosted_wifi_cipher_trans(uint32_t wifi_cipher)
{
...
/****************************************************************************
 * Name: esp_hosted_freq_to_channel
 *
 * Description:
...
static int esp_hosted_freq_to_channel(uint16_t freq)
{
  int channel = 0;
...
void esp_hosted_wifi_free_eb(void *eb)
{
  esp_hosted_wifi_internal_free_rx_buffer(eb);
}

/****************************************************************************
 * Name: esp_hosted_wifi_notify_subscribe
 *
 * Description:
...
int esp_hosted_wifi_notify_subscribe(pid_t pid, struct sigevent *event)
{
  int id;
  struct wifi_notify *notify;
  int ret = -1;
...
/****************************************************************************
 * Name: esp_hosted_wifi_adapter_init
 *
 * Description:
...
int esp_hosted_wifi_adapter_init(void)
{
  int ret;
  ...
  todo register esp_wifi_tx_done_cb
...
/****************************************************************************
 * Name: esp_hosted_wifi_sta_start
 *
 * Description:
 *   Start Wi-Fi station.
 *
...
int esp_hosted_wifi_sta_start(void)
{
  int ret;
  wifi_mode_t mode;
  ...
  ret = esp_hosted_wifi_set_mode(mode);
  if (ret)
    {
...
int esp_hosted_wifi_sta_stop(void)
{
  int ret;
  ...
  if (g_softap_started)
    {
      ret = esp_hosted_wifi_set_mode(WIFI_MODE_AP);
      if (ret)
        {
...
/****************************************************************************
 * Name: esp_hosted_wifi_sta_send_data
 *
 * Description:
...
int esp_hosted_wifi_sta_send_data(void *pbuf, uint32_t len)
{
  int ret;

  ret = esp_hosted_wifi_internal_tx(WIFI_IF_STA, pbuf, len);

  return wifi_errno_trans(ret);
}

/****************************************************************************
 * Name: esp_hosted_wifi_sta_register_recv_cb
 *
 * Description:
...
int esp_hosted_wifi_sta_register_recv_cb(int (*recv_cb)(void *buffer,
    uint16_t len,
    void *eb))
{
  int ret;

  ret = esp_hosted_wifi_internal_reg_rxcb(ESP_IF_WIFI_STA, (wifi_rxcb_t)recv_cb);

  return wifi_errno_trans(ret);
}

/****************************************************************************
 * Name: esp_hosted_wifi_sta_register_txdone_cb
 *
 * Description:
...
void esp_hosted_wifi_sta_register_txdone_cb(wifi_txdone_cb_t cb)
{
  g_sta_txdone_cb = cb;
}

/****************************************************************************
 * Name: esp_hosted_wifi_sta_read_mac
 *
 * Description:
...
int esp_hosted_wifi_sta_read_mac(uint8_t *mac)
{
  return esp_read_mac(mac, ESP_MAC_WIFI_STA);
}

/****************************************************************************
 * Name: esp_hosted_wifi_set_password
 *
 * Description:
...
int esp_hosted_wifi_sta_password(struct iwreq *iwr, bool set)
{
  int ret;
  int size;
  ...
  if (set)
    {
      ...
      if (g_sta_connected)
        {
          ret = esp_hosted_wifi_sta_disconnect();
          if (ret)
            {
          ...
          ret = esp_hosted_wifi_set_config(WIFI_IF_STA, &wifi_cfg);
          if (ret)
            {
          ...
          ret = esp_hosted_wifi_sta_connect();
          if (ret)
            {
      ...
      if (g_sta_connected)
        {
          ...
          ret = esp_hosted_wifi_sta_get_ap_info(&ap_info);
          if (ret)
            {
...
/****************************************************************************
 * Name: esp_hosted_wifi_sta_essid
 *
 * Description:
...
int esp_hosted_wifi_sta_essid(struct iwreq *iwr, bool set)
{
  int ret;
  int size;
  wifi_config_t wifi_cfg;
...
/****************************************************************************
 * Name: esp_hosted_wifi_sta_bssid
 *
 * Description:
...
int esp_hosted_wifi_sta_bssid(struct iwreq *iwr, bool set)
{
  int ret;
  wifi_config_t wifi_cfg;
...
/****************************************************************************
 * Name: esp_hosted_wifi_sta_connect
 *
 * Description:
...
int esp_hosted_wifi_sta_connect(void)
{
  int ret;
  uint32_t ticks;
  ...
  ret = esp_hosted_wifi_set_config(WIFI_IF_STA, &g_sta_wifi_cfg);
  if (ret)
    {
      wlerr("Failed to set Wi-Fi config data ret=%d\n", ret);
      return wifi_errno_trans(ret);
...
/****************************************************************************
 * Name: esp_hosted_wifi_sta_disconnect
 *
 * Description:
...
int esp_hosted_wifi_sta_disconnect(void)
{
  int ret;
  ...
  ret = esp_hosted_wifi_disconnect();
  if (ret)
    {
...
int esp_hosted_wifi_sta_mode(struct iwreq *iwr, bool set)
{
  if (set == false)
    {
...
int esp_hosted_wifi_sta_auth(struct iwreq *iwr, bool set)
{
  int ret;
  int cmd;
  wifi_config_t wifi_cfg;
  ...
  if (set)
  ...
  else
    {
      ...
      switch (cmd)
        {
        case IW_AUTH_WPA_VERSION:
          iwr->u.param.value = esp_hosted_wifi_auth_trans(ap_info.authmode);
          break;

        case IW_AUTH_CIPHER_PAIRWISE:
          iwr->u.param.value =
            esp_hosted_wifi_cipher_trans(ap_info.pairwise_cipher);
          break;

        case IW_AUTH_CIPHER_GROUP:
          iwr->u.param.value = esp_hosted_wifi_cipher_trans(ap_info.group_cipher);
          break;
...
int esp_hosted_wifi_sta_freq(struct iwreq *iwr, bool set)
{
  int ret;
  ...
  if (set && (iwr->u.freq.flags == IW_FREQ_FIXED))
    {
      ...
      wifi_cfg.sta.channel = esp_hosted_freq_to_channel(iwr->u.freq.m);
...
/****************************************************************************
 * Name: esp_hosted_wifi_sta_bitrate
 *
 * Description:
...
int esp_hosted_wifi_sta_bitrate(struct iwreq *iwr, bool set)
{
  int ret;
  wifi_ap_record_t ap_info;
...
/****************************************************************************
 * Name: esp_hosted_wifi_sta_get_txpower
 *
 * Description:
...
int esp_hosted_wifi_sta_txpower(struct iwreq *iwr, bool set)
{
  int ret;
  int8_t power;
  double power_dbm;
  ...
  if (set)
    {
      ...
      esp_hosted_wifi_set_max_tx_power(power);
      return OK;
    }
  else
    {
      ret = esp_hosted_wifi_get_max_tx_power(&power);
      if (ret)
        {
...
/****************************************************************************
 * Name: esp_hosted_wifi_sta_get_channel_range
 *
 * Description:
...
int esp_hosted_wifi_sta_channel(struct iwreq *iwr, bool set)
{
  int ret;
  int k;
  ...
  if (set)
    ...
    {
      ret = esp_hosted_wifi_get_country(&country);
      if (ret)
        {
...
/****************************************************************************
 * Name: esp_hosted_wifi_sta_country
 *
 * Description:
...
int esp_hosted_wifi_sta_country(struct iwreq *iwr, bool set)
{
  int ret;
  char *country_code;
  wifi_country_t country;
  ...
  if (set)
    {
      ...
      ret = esp_hosted_wifi_set_country(&country);
      if (ret)
        {
...
int esp_hosted_wifi_sta_rssi(struct iwreq *iwr, bool set)
{
  int ret;
  wifi_ap_record_t ap_info;
...
/****************************************************************************
 * Name: esp_hosted_wifi_softap_start
 *
 * Description:
 *   Start Wi-Fi SoftAP.
 *
...
int esp_hosted_wifi_softap_start(void)
{
  int ret;
  wifi_mode_t mode;
...
/****************************************************************************
 * Name: esp_hosted_wifi_softap_stop
 *
 * Description:
 *   Stop Wi-Fi SoftAP.
 *
...
int esp_hosted_wifi_softap_stop(void)
{
  int ret;
  ...
  if (g_sta_started)
    {
      ret = esp_hosted_wifi_set_mode(WIFI_MODE_STA);
      if (ret)
        {
          wlerr("Failed to set Wi-Fi AP mode ret=%d\n", ret);
          ret = wifi_errno_trans(ret);
...
/****************************************************************************
 * Name: esp_hosted_wifi_softap_send_data
 *
 * Description:
...
int esp_hosted_wifi_softap_send_data(void *pbuf, size_t len)
{
  int ret;

  ret = esp_hosted_wifi_internal_tx(WIFI_IF_AP, pbuf, len);

  return wifi_errno_trans(ret);
}

/****************************************************************************
 * Name: esp_hosted_wifi_softap_register_recv_cb
 *
 * Description:
...
int esp_hosted_wifi_softap_register_recv_cb(int (*recv_cb)(void *buffer,
    uint16_t len,
    void *eb))
{
  int ret;

  ret = esp_hosted_wifi_internal_reg_rxcb(ESP_IF_WIFI_AP, (wifi_rxcb_t)recv_cb);

  return wifi_errno_trans(ret);
}

/****************************************************************************
 * Name: esp_hosted_wifi_softap_register_txdone_cb
 *
 * Description:
...
void esp_hosted_wifi_softap_register_txdone_cb(wifi_txdone_cb_t cb)
{
  g_softap_txdone_cb = cb;
}

/****************************************************************************
 * Name: esp_hosted_wifi_softap_read_mac
 *
 * Description:
...
int esp_hosted_wifi_softap_read_mac(uint8_t *mac)
{
  return esp_read_mac(mac, ESP_MAC_WIFI_SOFTAP);
}

/****************************************************************************
 * Name: esp_hosted_wifi_softap_password
 *
 * Description:
...
int esp_hosted_wifi_softap_password(struct iwreq *iwr, bool set)
{
  int ret;
  int size;
  wifi_config_t wifi_cfg;
  ...
  if (set)
    {
      ...
      if (g_softap_started)
        {
          ret = esp_hosted_wifi_set_config(WIFI_IF_AP, &wifi_cfg);
          if (ret)
            {
              wlerr("Failed to set Wi-Fi config data ret=%d\n", ret);
              return wifi_errno_trans(ret);
...
/****************************************************************************
 * Name: esp_hosted_wifi_softap_essid
 *
 * Description:
...
int esp_hosted_wifi_softap_essid(struct iwreq *iwr, bool set)
{
  int ret;
  int size;
  wifi_config_t wifi_cfg;
...
/****************************************************************************
 * Name: esp_hosted_wifi_softap_bssid
 *
 * Description:
...
int esp_hosted_wifi_softap_bssid(struct iwreq *iwr, bool set)
{
  return -ENOSYS;
}

/****************************************************************************
 * Name: esp_hosted_wifi_softap_connect
 *
 * Description:
...
int esp_hosted_wifi_softap_connect(void)
{
  int ret;

  ret = esp_hosted_wifi_set_config(WIFI_IF_AP, &g_softap_wifi_cfg);
  if (ret)
    {
      wlerr("Failed to set Wi-Fi config data ret=%d\n", ret);
      return wifi_errno_trans(ret);
...
/****************************************************************************
 * Name: esp_hosted_wifi_softap_disconnect
 *
 * Description:
...
int esp_hosted_wifi_softap_disconnect(void)
{
  return OK;
}

/****************************************************************************
 * Name: esp_hosted_wifi_softap_mode
 *
 * Description:
...
int esp_hosted_wifi_softap_mode(struct iwreq *iwr, bool set)
{
  if (set == false)
    {
...
/****************************************************************************
 * Name: esp_hosted_wifi_softap_auth
 *
 * Description:
...
int esp_hosted_wifi_softap_auth(struct iwreq *iwr, bool set)
{
  int ret;
  int cmd;
  wifi_config_t wifi_cfg;
...
/****************************************************************************
 * Name: esp_hosted_wifi_softap_freq
 *
 * Description:
...
int esp_hosted_wifi_softap_freq(struct iwreq *iwr, bool set)
{
  int ret;
  wifi_config_t wifi_cfg;
  ...
  if (set)
    {
      int channel = esp_hosted_freq_to_channel(iwr->u.freq.m);
...
/****************************************************************************
 * Name: esp_hosted_wifi_softap_get_bitrate
 *
 * Description:
...
int esp_hosted_wifi_softap_bitrate(struct iwreq *iwr, bool set)
{
  return -ENOSYS;
}

/****************************************************************************
 * Name: esp_hosted_wifi_softap_txpower
 *
 * Description:
...
int esp_hosted_wifi_softap_txpower(struct iwreq *iwr, bool set)
{
  return esp_hosted_wifi_sta_txpower(iwr, set);
}

/****************************************************************************
 * Name: esp_hosted_wifi_softap_channel
 *
 * Description:
...
int esp_hosted_wifi_softap_channel(struct iwreq *iwr, bool set)
{
  return esp_hosted_wifi_sta_channel(iwr, set);
}

/****************************************************************************
 * Name: esp_hosted_wifi_softap_country
 *
 * Description:
 *   Configure country info.
 *
...
int esp_hosted_wifi_softap_country(struct iwreq *iwr, bool set)
{
  return esp_hosted_wifi_sta_country(iwr, set);
}

/****************************************************************************
 * Name: esp_hosted_wifi_softap_rssi
 *
 * Description:
 *   Get Wi-Fi sensitivity (dBm).
 *
...
int esp_hosted_wifi_softap_rssi(struct iwreq *iwr, bool set)
{
  return -ENOSYS;
}
...
/****************************************************************************
 * Name: esp_hosted_wifi_stop_callback
 *
 * Description:
...
void esp_hosted_wifi_stop_callback(void)
{
  wlinfo("INFO: Try to stop Wi-Fi\n");

  int ret = esp_hosted_wifi_stop();
  if (ret)
    {
